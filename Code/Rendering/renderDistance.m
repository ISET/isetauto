...
lensType = {'dgauss.22deg.12.5mm'}; % {'fisheye.87deg.12.5mm'};%{'wide.40deg.12.5mm'}; %,,'dgauss.22deg.12.5mm'};
mode = {'radiance','mesh'};
...
hints.recipeName = 'City-Car-Distance'; % Name of the render
...
% Copy fog parameters
waves = 300:10:800;
abs_fog = linspace(0.009,0.01,length(waves));
rtbWriteSpectrumFile(waves,abs_fog,fullfile(resourceFolder,'abs_fog.spd'));

[vsf_fog, sct_fog, waves] = calculateScattering(0.5,0.5);
sct_fog = linspace(0.08,0.07,length(waves));
