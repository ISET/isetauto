...
if exist(destDir,'dir') == false
    ...
    for q=1:length(quality)
        mkdir(fullfile(destDir,sprintf('sRgb_%2i',quality(q))));
    end
...
for f=1:length(fileNames)
    ...
    try
        ...
        for q=1:length(quality)
            imwrite(demosaicedImage,fullfile(destDir,sprintf('sRgb_%2i',quality(q)),sprintf('%05i.jpg',cntr)),'Quality',quality(q));
        end
